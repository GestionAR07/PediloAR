...
export default async function CustomerOrderDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="rounded-[1.75rem] border border-violet-100/80 bg-white p-6 shadow-soft">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <span
            className={`w-fit rounded-full px-4 py-2 text-sm font-extrabold ${view.active ? "bg-violet-100 text-violet-800" : view.status === "CANCELED" ? "bg-rose-100 text-rose-800" : "bg-emerald-100 text-emerald-800"}`}
          >
            {view.statusLabel}
          </span>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.8fr)]">
        <div className="space-y-6">
          <section className="rounded-[1.75rem] border border-violet-100/80 bg-white p-6 shadow-soft">
            ...
            <ol className="mt-5 space-y-5">
              {view.timeline.map((event, index) => (
                <li
                  ...
                  className="relative pl-8 before:absolute before:top-2 before:left-1 before:h-3 before:w-3 before:rounded-full before:bg-violet-600 after:absolute after:top-5 after:bottom-[-1.5rem] after:left-[0.55rem] after:w-px after:bg-violet-200 last:after:hidden"
                >
        ...
        <aside className="space-y-4">
          {view.delivery ? (
            <section className="rounded-[1.5rem] border border-violet-100/80 bg-white p-5 shadow-soft">
